...
public class DiskRTree implements RTree {
	...
	public void newIndex(File file) throws IOException {
		if (file.exists()) {
			throw new IOException(I18N.get("File already exists"));
		} else {
			inMemory = false;
	...
	public void openIndex(File file) throws IOException {
		if (!file.exists()) {
			throw new IOException(I18N.get("The file {0}" + " does not exist",
					file));
		} else {
			inMemory = false;
			fis = new RandomAccessFile(file, "rw");
	...
	private byte[] readNodeBytes(int dir) throws IOException {
		...
		System.arraycopy(thisBlockBytes, 0, nodeBytes, 0, thisBlockBytes.length);
		...
		if (node == null) {
			...
			case LEAF:
				...
				try {
					...
					throw new IOException(I18N.get("Cannot understand the "
							+ "geometry bytes: {0}", e.getMessage()));
				}
				break;
			case INTERIOR_NODE:
				// Return the instance
			...
			default:
				throw new IOException(I18N.get(
						"Cannot understand block type:{0}", blockType));
			}
	...
	public void save() throws IOException {
		if (inMemory) {
			throw new UnsupportedOperationException(I18N.get("Memory "
	...
	public void checkTree() throws IOException {
		...
		if (size() != root.getAllValues().length) {
			throw new RuntimeException(I18N.get("size inconsistency"));
		}
	}
	...
	public Envelope getRange() throws IOException {
		Envelope env = root.getEnvelope();
		if (env != null) {
			return env;
		} else {
			return null;
