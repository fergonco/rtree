...
public class ReadWriteBufferManager {
	...
	private void readIntoBuffer(long position, int bufferSize)
			throws IOException {
	...
	/**
	 * Gets len bytes at the specified position and stores in buffer starting at
	 * the zero position
	 * 
	 * @param pos
	 * @param buffer
	 ...
	 * @throws IOException
	 */
	public ByteBuffer get(int pos, byte[] buffer, int len) throws IOException {
		int windowOffset = getWindowOffset(pos, buffer.length);
		this.buffer.position(windowOffset);
		return this.buffer.get(buffer, 0, len);
	}
	...
	public void put(byte[] bytes, int off, int len) throws IOException {
		int windowOffset = getWindowOffset(getPosition(), len);
