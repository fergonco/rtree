...
public class RTreeLeaf extends AbstractRTreeNode implements RTreeNode {
	...
	public byte[] getBytes() throws IOException {
		...
		// Write a coordinates of all envelopes
		Envelope[] used = geometries.toArray(new Envelope[0]);
		for (int i = 0; i < used.length; i++) {
			dos.writeDouble(used[i].getMinX());
			dos.writeDouble(used[i].getMinY());
			dos.writeDouble(used[i].getMaxX());
			dos.writeDouble(used[i].getMaxY());
		}
	...
	public static RTreeLeaf createLeafFromBytes(DiskRTree tree, int dir,
		...
		// Read the values
		ret.geometries = new ArrayList<Envelope>(valueCount);
